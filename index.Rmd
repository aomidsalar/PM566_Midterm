...
# Methods
The dataset I used is California's Quarterly Census of Employment and Wages from data.gov (https://catalog.data.gov/dataset/quarterly-census-of-employment-and-wages-qcew). It contains data categorized by the following variables:
* Area type (county, state-wide, US)
* Area name (name of county)
* Year (2004 - 2021)
* Quarter
* Ownership (state government, federal government, private)
* NAICS (North American Industry Classification System) level
* NAICS code
* Industry name
* Number of Establishments
* Average monthly employment
* 1st, 2nd and 3rd month employment (if there is a quarter designation)
* Total wages for all workers
...
The counties with the lowest median wages are Sierra County, Alpine County and Trinity County. These are the least populated counties in California and are in more mountainous areas in Northern and Northeastern California. The wages being so low in these areas likely correspond to seasonal jobs (more jobs in the winter months), and perhaps more part-time or volunteer workers since there are no major cities in these counties. 
...
edsector[order(-edsector$median_wage), ] %>% head(5) %>% select(`Area Name`, `Industry Name`, `Year`, median_wage) %>% knitr::kable(caption = "5 Counties with the Highest Median Weekly Wages per Year")
edsector[order(edsector$median_wage), ] %>% head(5) %>% select(`Area Name`, `Industry Name`, `Year`, median_wage) %>% knitr::kable(caption = "5 Counties with the Highest Median Weekly Wages per Year")
